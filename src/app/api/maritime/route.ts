...
function localizeShipName(ship: any): string {
  ...
  if (!name || /^\d+$/.test(name)) {
    ...
    if (lat >= 33 && lat <= 39 && lng >= 124 && lng <= 131) {
      if (ship.type === 'military') return `연안 해경 경비정 ${numSuffix}호`;
      if (ship.type === 'tanker') return `연안 유조선 ${numSuffix}호`;
      return `연안 화물선 ${numSuffix}호`;
    }
    return `해상 운항선 ${numSuffix}호`;
  }
...
function buildSnapshot(now: number): string {
  for (const [mmsi, ship] of shipsCache.entries()) {
    if (now - ship.timestamp > 30 * 60 * 1000) {
...
function generateCirclePolygon(centerLng: number, centerLat: number, radiusKm: number, points = 32): [number, number][] {
  ...
  // Derive real dark targets from real vessels in shipsCache that recently went silent while underway
  const darkTargets: any[] = [];
  for (const ship of shipsCache.values()) {
    const elapsedSec = (now - ship.timestamp) / 1000;
    // Moving vessel (> 3 knots) whose signal ceased within 3 to 30 minutes
    if (ship.speed > 3 && elapsedSec >= 180 && elapsedSec <= 1800 && ship.lat && ship.lng) {
      const elapsedHours = elapsedSec / 3600;
      const radiusKm = Math.min(Math.round(ship.speed * 1.852 * elapsedHours * 10) / 10, 50);
      darkTargets.push({
        mmsi: ship.mmsi,
        name: ship.name ? `${ship.name} (신호 두절 감지)` : `MMSI ${ship.mmsi} (신호 두절)`,
        flag: ship.country || 'UNKNOWN',
        type: ship.type || 'cargo',
        last_known_lat: ship.lat,
        last_known_lng: ship.lng,
        last_speed_knots: ship.speed,
        time_since_dark_sec: Math.round(elapsedSec),
        radius_km: radiusKm,
        risk: ship.speed > 15 ? 'CRITICAL' : 'HIGH',
        description: `AIS 신호 수신 두절 (${Math.round(elapsedSec / 60)}분 경과). 최종 속력 ${ship.speed}kt 기준 키네틱 불확실성 반경 ${radiusKm}km.`,
        kinetic_bubble: generateCirclePolygon(ship.lng, ship.lat, radiusKm)
      });
      if (darkTargets.length >= 10) break;
    }
  }

  const darkFleetGeojson = {
    type: 'FeatureCollection',
    features: darkTargets.flatMap(t => [
      {
        type: 'Feature',
        geometry: {
          type: 'Polygon',
          coordinates: [t.kinetic_bubble]
        },
        properties: {
          mmsi: t.mmsi,
          name: t.name,
          flag: t.flag,
          type: t.type,
          lost_hours_ago: Math.round(t.time_since_dark_sec / 360) / 10,
          last_speed_knots: t.last_speed_knots,
          kinetic_radius_km: t.radius_km,
          risk_assessment: t.description,
          is_bubble: true
        }
      },
      {
        ...
        geometry: {
          type: 'Point',
          coordinates: [t.last_known_lng, t.last_known_lat]
        },
        properties: {
          mmsi: t.mmsi,
          name: t.name,
  ...
  const temporal = {
    observed_at: new Date(ships[0]?.timestamp || now).toISOString(),
    fetched_at: new Date(now).toISOString(),
    next_update_at: nextUpdateAt,
    interval_seconds: updateIntervalSec,
    staleness: 'FRESH' as const,
    source_name: 'aisstream.io (Global WebSocket AIS Live)',
    rate_limit_info: '실시간 WebSocket 피드 (5초 캐시 스냅샷 / 16,000+ 실제 선박)'
  };
  ...
  const payload: any = {
    ...
    total_dark_fleet: darkTargets.length,
    temporal,
...
export async function GET() {
  try {
    ...
    console.error('[OSIRIS Maritime GET Error]:', error);
